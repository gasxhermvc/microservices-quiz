...
func (log *PatternLogger) InitLogger(productName, appName string, sourceSystem LogSystem, targetSys LogSystem) *PatternLogger {
	return log.initLogger(productName, sourceSystem, targetSys, appName)
}
...
func (log *PatternLogger) checkArguments(args []interface{}) (message string, staceTrace string) {
	for i := 0; i < len(args); i++ {
		...
		case string:
			message = message + " " + v
		case error:
			staceTrace = staceTrace + " " + log.getStackTraceError(v)
